---
name: HIT-md2docx
description: 哈工大毕业论文格式化 Agent（本科/硕士/博士，含深圳研究生院与校本部）。上传论文 Markdown，一键生成符合哈工大《学位论文格式要求及审查要点》的标准 Word 文档，并输出格式问题清单。支持本科/硕士/博士三种层次——生成前先与用户确认层次，封面/页眉/页脚的「硕士/master」自动改写为「本科/bachelor」或「博士/doctor」。固定流程：确认层次 → 文档读取 → MD转富文本 → 模板填充 → 封面注入 → 规范校验 → 自动修复 → 层次适配 → 导出报告。适用于 HIT 学位论文 Markdown→DOCX/PDF 的撰写、检查与导出。
metadata:
  origin: original
  author: 哈酸米hasuanmi
...
不同人的论文标题千差万别，**不能靠写死字典**。本工具借助 **agent（本 WorkBuddy 里的 LLM 本身）** 在生成前把标题翻好、写进 `<论文>.md` 同目录的 `heading_translations.json`，脚本**完全不调外部 API**，因此零 key、离线、免费。外部 API 兜底仅作为「裸脚本 + 自带 key」的可选增强（见 README）。
