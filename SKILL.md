...
metadata:
  origin: original
  author: 哈酸米hasuanmi
  author-profile: https://github.com/hasuanmi
...
本 Skill 由 **哈酸米hasuanmi** 原创，仓库：<https://github.com/hasuanmi/HIT-md2docx>。欢迎关注作者，获取更多 AI / Agent / Skill 与工作流分享。

## 适用范围与输入输出

**适用**：以 Markdown 撰写的哈工大（含深圳研究生院与校本部）本科/硕士/博士论文，一键生成符合《学位论文格式要求及审查要点》的标准 Word 文档 + 格式问题清单。

**边界（不做什么）**：
- 只做格式与排版，**不撰写、不润色论文内容**；仅支持哈工大模板，不通用其他学校。
- PDF 仅为预览，**交付物是 docx**；最终仍需在 Word/WPS 人工复核（刷新目录域、分页、图/表/公式观感）。
- 公式渲染依赖 `math/latex2omml_node`（需 `npm install`）；缺失时公式退化为纯文本，属已知限制。

**输入要求**：
- 一篇 GFM Markdown，结构参照 `skill/references/example-md.md`（至少含 H1 题名、摘要、各章、参考文献）。
- 封面信息 / 原创声明由 `input/front_matter_hit.md` **自动注入**，无需手写；题名自动从 H1 同步。
- 英文目录默认由 agent 自带 LLM 零 key 生成；若裸跑 `generate.py` 且无预填字典、无 API key，英文目录优雅回退中文（不报错）。

**输出要求**：
- `output/<论文>.docx`：含封面 / 原创声明 / 中英文摘要 / 目录（可选英文 Contents）/ 正文 / 参考文献，套用 HIT 自定义样式。
- `问题清单.md`：机检 R01–R11 的可读报告，供人工逐项核对。
...
1. **范例 MD 样板** — `skill/references/example-md.md`：标准论文 Markdown 结构范本（入口即照此组织输入）。
2. **范例 docx 规范** — `skill/references/template-spec.md`：哈工大硕士格式规范（字体/字号/行距/页眉/编号/版芯），含 HIT 自定义样式 id 对照。
3. **自查清单** — `skill/references/selfcheck-checklist.md`（权威机检映射）+ `skill/references/hit-master-review-points.md`（完整审查要点与往届 10 大易错点）。
...
不同人的论文标题千差万别，**不能靠写死字典**。本工具借助 **运行本 skill 的 agent（无论 WorkBuddy / Cursor / 其他任意 IDE 里的 LLM 本身）** 在生成前把标题翻好、写进 `<论文>.md` 同目录的 `heading_translations.json`，脚本**完全不调外部 API**，因此**零 key、离线、免费、且与 IDE 无关**（整条链路只是「Python 导出标题 → agent 翻译 → 写一个 json → 引擎读 json」，没有任何 IDE 专有依赖）。

> 这正是你要的「用户不管用哪个 IDE 都能自动翻译」：**只要 skill 是被 agent 按本流程跑的，英文目录就全自动出，用户无需配任何 key。**
> 若用户改用「裸脚本 + 自带 OpenAI 兼容 key」模式，从 1.0.5 起**只需填 `HITMD2DOCX_LLM_API_KEY` 一个变量**即可（`base_url`/`model` 有 OpenAI 兼容默认值）；这是 agent 路径之外的可选增强，不配 key 则不触发任何网络请求。
...
1. **导出待翻译标题**（复用引擎自己的预处理 + `collect_toc_entries`，输出的 key 与引擎查表 key 完全一致，剥号/剥行内英文/跳过已有翻译）：
   ...
   # 输出形如：{"1.1 研究背景": "", "1.2 研究意义": "", "参考文献": "", ..., "<论文总标题>": ""}
   # 注意：论文总标题也在其中（用于封面英文题），务必一并翻译。
   ```
...
- `skill/references/example-md.md` · `skill/references/template-spec.md` · `skill/references/selfcheck-checklist.md` · `skill/references/hit-master-review-points.md`
- `skill/references/markdown-usage.md` · `skill/references/pdf-backends.md` · `skill/references/troubleshooting.md`
